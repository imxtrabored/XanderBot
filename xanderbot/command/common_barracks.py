...
async def callback_save(custom_name, hero, user_id):
    hero.custom_name = custom_name
    status = UnitLib.insert_custom_hero(hero, user_id)
    if status == 0:
        return ReplyPayload(content=f'Build saved as "{custom_name}"!')
    if status == 1:
        return ReplyPayload(
            replyable=UserPrompt(
                callback=callback_save,
                content=('Name is invalid (matches normal hero). '
                         'Enter a new name:'),
                data=hero
            )
        )
    if status == 2:
        return ReplyPayload(
            content=('Your barracks is full (300)! Please send heroes home '
                     'before adding new heroes. (``delete`` or ``sendhome``.)')
        )
    if status == 3:
        return ReplyPayload(
            replyable=UserPrompt(
                callback=callback_overwrite,
                content=('A hero with that name or a similar name is already '
                         'in your barracks.  Overwrite it? (Y/N):'),
                data=hero
            )
        )
